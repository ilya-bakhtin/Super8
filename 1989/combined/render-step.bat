...
SET dir_f_ds=%dir_f:\=\\%
    ...
    "cmd.exe" /c ""C:\Users\ilx\wrk\ffmpeg\ffmpeg.exe"             -loglevel level+error -i "%2.avs" -strict -1 -f yuv4mpegpipe - | "C:\Users\ilx\wrk\x265\x265.exe"                    --crf 17 --preset=slow --tune grain --sar 1:1 -o "%1\%2.h265" --y4m -"
...
mkdir "%dir_f%"
    "C:\Program Files\GPAC\mp4box.exe"               -add "%1\%2.h265#trackID=1:fps=60.0:par=1:1:name=" -add "%1\%2.m4a#trackID=1:name=" -tmp "%dir_f_ds%" -new "%1\%2.mp4"
rem "D:\Program Files\MeGUI\tools\mp4box\mp4box.exe" -add "%1\%2.h265#trackID=1:fps=60.0:par=1:1:name=" -add "%1\%2.m4a#trackID=1:name=" -tmp "%dir_f_ds%" -new "%1\%2.mp4"
rmdir "%dir_f%"
